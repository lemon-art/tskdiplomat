...
?>
		...
		<div class="bx_ordercart_order_pay_center">
			...
			<?endif;?>
                    <div id='order-min-value' class="order-min-value <?=($arResult['allSum'] < $GLOBALS['MIN_ORDER_SUM'])?'min-value':''?>">            
                        <div class="min-order-message">
                            <span class="min-order-sum-message">Минимальная сумма заказа <b><?=number_format($GLOBALS['MIN_ORDER_SUM'],0,'.',' ');?></b> рублей.</span>
                            <span class="checkout-disabled"><?=GetMessage("SALE_ORDER")?></span>
                        </div>    
...
BX.addCustomEvent('OnBasketChange', function(){
    if(BX('allSum_FORMATED')){
        sum = parseFloat(BX('allSum_FORMATED').innerHTML.replace('&nbsp;',''));
        console.log(sum);
        
        if(sum < <?=$GLOBALS['MIN_ORDER_SUM']?>){
               BX.addClass(BX('order-min-value'),'min-value'); 
               console.log('OnBascetChange disable order');
            }else{
               BX.removeClass(BX('order-min-value'),'min-value'); 
               console.log('OnBascetChange enable order');
        }    
    }
});
</script>
