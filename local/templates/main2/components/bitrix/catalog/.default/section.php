...
?>
<div class="row">
<div class="col-left sidebar col-md-3 col-sm-4 hidden-xs">
                <?//trace($arCurSection)?>   
...
<?//endif;?>
		...
		$APPLICATION->IncludeComponent(
			...
			if(($APPLICATION->GetCurPage() == $APPLICATION->GetCurPageParam()) && (!strpos($APPLICATION->GetCurPageParam(), "/f/"))){//главная страница - ничего не делаем
			
			}elseif(!strpos($APPLICATION->GetCurPageParam(), "/f/")){//Пагинация + сортировка + отображение + фильтр
				$APPLICATION->SetPageProperty("canonical", "http://".$_SERVER["SERVER_NAME"].$APPLICATION->GetCurPage());
				
			}else{
				if($arResult["VARIABLES"]["SMART_FILTER_PATH"] != "" && strpos($_SERVER["SCRIPT_URL"], "/f/")){
					$APPLICATION->SetPageProperty("canonical", "http://".$_SERVER["SERVER_NAME"].explode("f/", $APPLICATION->GetCurPage())[0]);
				}
			}
...
<div class="col-md-9">
<?$APPLICATION->IncludeComponent("bitrix:advertising.banner", "catalog_section_top", Array(
	"COMPONENT_TEMPLATE" => ".default",
		"TYPE" => "catalog_section_top",	// Тип баннера
...
<?//endif;?>
	...
	//при сброшенном фильтре все равно выводим - просбма клиента
	if(in_array("f", explode("/", $APPLICATION->GetCurPage())) && in_array("clear", explode("/", $APPLICATION->GetCurPage()))){
		echo $GLOBALS['SECTION_DESCRIPTION'];
...
<!-- catalog section
<?//trace($GLOBALS[$arParams["FILTER_NAME"]])?>
<?//trace($arResult)?>
<?//trace($arParams)?>
<?//trace($arCurSection)?>
-->
...
<?
	...
	if(isset($sotbitSeoMetaBreadcrumbLink) && !empty($sotbitSeoMetaBreadcrumbTitle)) {
