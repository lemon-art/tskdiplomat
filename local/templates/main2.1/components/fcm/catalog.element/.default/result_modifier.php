...
if(count($arResult["PROPERTIES"]["LOOK_LIKE"]["VALUE"]) > 0){
	$arResult["LOOK_LIKE"] = array();
	$prop = $arResult["PROPERTIES"]["LOOK_LIKE"];
	$arSelect = array("ID","IBLOCK_ID","NAME","PREVIEW_TEXT","DETAIL_PICTURE","DETAIL_PAGE_URL");
	...
	$arFilter = array(
		...
		while($arElement = $res->GetNext()){
			...
			$arResult["LOOK_LIKE"][] = $arElement;
		}
}
if(count($arResult["PROPERTIES"]["SOPUT_PRODUCTS"]["VALUE"]) > 0){
	$arResult["SOPUT_PRODUCTS"] = array();
	$prop = $arResult["PROPERTIES"]["SOPUT_PRODUCTS"];
	$arSelect = array("ID","IBLOCK_ID","NAME","PREVIEW_TEXT","DETAIL_PICTURE","DETAIL_PAGE_URL");
	...
	$arFilter = array(
		...
		while($arElement = $res->GetNext()){
			...
			$arResult["SOPUT_PRODUCTS"][] = $arElement;
		}
}
