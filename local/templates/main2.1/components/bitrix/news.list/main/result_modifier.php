...
if(intval($arParams['PREVIEW_IMAGE_WIDTH']) <= 0 ) $arParams['PREVIEW_IMAGE_WIDTH'] = 230;
if(intval($arParams['PREVIEW_IMAGE_HEIGHT']) <= 0 ) $arParams['PREVIEW_IMAGE_HEIGHT'] = 172;
...
if (is_array($arResult["ITEMS"]) AND !empty($arResult["ITEMS"])):
    foreach ($arResult["ITEMS"] as $key => $arItem){
            ...
            if ($arItem['DETAIL_PICTURE']['ID'] > 0) {
                $image = CFile::ResizeImageGet($arItem['DETAIL_PICTURE']['ID'], array('width'=>$arParams['PREVIEW_IMAGE_WIDTH'], 'height'=>$arParams['PREVIEW_IMAGE_HEIGHT']), BX_RESIZE_IMAGE_EXACT, true); 
                $arItem['PREVIEW_PICTURE']['SRC'] = $image['src'];
                $arItem['PREVIEW_PICTURE']['WIDTH'] = $image['width'];
                $arItem['PREVIEW_PICTURE']['HEIGHT'] = $image['height'];
            }
