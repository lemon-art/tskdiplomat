...
if($props_array['UF_ZH1']){
	...
	$myh1h1h1 = $props_array['UF_ZH1'];
}
else{
	//echo $arResult["NAME"];
	$myh1h1h1 = $arResult["NAME"];
}

$APPLICATION->AddViewContent("myh1h1h1",$myh1h1h1); 
?>
...
<?if($arResult['ID']==582) {
$arElement["NAME"] = str_replace('Кнауф','Гипсокартон Кнауф', $arElement["NAME"]);
$arElement["NAME"] = str_replace('Волма','Гипсокартон Волма', $arElement["NAME"]);
}
?>
<?if($arResult['ID']==583) {
$arElement["NAME"] = str_replace('Кнауф','Гипсоволокно Кнауф', $arElement["NAME"]);
}
?>
<?if($arResult['ID']==448) {
$arElement["NAME"] = str_replace('-КРОВЛЯ','', $arElement["NAME"]);
$arElement["NAME"] = str_replace('– кровля','', $arElement["NAME"]);
$arElement["NAME"] = "Кровельный дюбель ".$arElement["NAME"];
}
?>
...
<?if($ar_res2["NAME"] && $catb == 24){?>
<p style="    padding: 5px 0 0 0;">
<b>Бренд:</b> <a href="<? echo $ar_res2["SECTION_PAGE_URL"];?>"><? echo $ar_res2["NAME"];?></a>
</p>
<?}
elseif ($arElement['PROPERTIES']['MANUFACTURER']['VALUE'])
{
$url='';
$title= $arElement['PROPERTIES']['MANUFACTURER']['VALUE'];
if ($title == "ЕЗСМ") $url =  "http://tskdiplomat.ru/catalog/gazosilikatnye_bloki_ezsm/";
if ($title == "ГКЛ") $url =  "http://tskdiplomat.ru/catalog/gkl/";
if ($title == "Магма") $url =  "http://tskdiplomat.ru/catalog/magma_sukhie_smesi/";
if ($title == "Forman") $url =  "http://tskdiplomat.ru/catalog/forman/";
if ($title == "INDASTRO") $url =  "http://tskdiplomat.ru/catalog/indastro/";
if ($title == "GLIMS-Elastic") $url =  "http://tskdiplomat.ru/catalog/glims/";
if ($title == "GLIMS ВодоStop") $url =  "http://tskdiplomat.ru/catalog/glims/";
if ($title == "GLIMS-RealFix") $url =  "http://tskdiplomat.ru/catalog/glims/";
if ($title == "Unis") $url =  "http://tskdiplomat.ru/catalog/unis/";
if ($title == "Основит") $url =  "http://tskdiplomat.ru/catalog/osnovit/";
if ($title == "Perfekta") $url =  "http://tskdiplomat.ru/catalog/perfect_sense/";

if ($url) {
?>
<p style="    padding: 5px 0 0 0;">
<b>Бренд:</b> <a href="<? echo $url;?>"><? echo $title;?></a>
</p>
...
<?
//echo $arResult['ID'];
if ($arResult['ID'] == 223 || $arResult['ID'] == 26 || $arResult['ID'] == 24 || $arResult['ID'] == 36 )
{
/*$cat22 = $arElement["~IBLOCK_SECTION_ID"];
$res22 = CIBlockSection::GetByID($cat22);
...
if($ar_res22["NAME"] ):?>
<p style="    padding: 5px 0 0 0;">
<b>Бренд:</b> <a href="<? echo $ar_res22["SECTION_PAGE_URL"];?>"><? echo $ar_res22["NAME"];?></a>
</p>
<?endif;?>
<?  //print_r($arElement['PROPERTIES']['MANUFACTURER']['VALUE']); ?>
...
*/?>
   <?if($arElement['PROPERTIES']['TOORDER']['VALUE_XML_ID'] == 'TOORDER_YES'):?>
    ...
    <?else:?>
        ...
        <?endif?>
    <?endif?>
<input type="button" value="<?php echo GetMessage("CATALOG_ADD"); ?>" class="ajax_add_to_cart_button exclusive" onclick="yaCounter22073026.reachGoal('buyFromSection');ga('send', 'event', 'buyFromSection', 'submit', 'Купить разводящая');" rel="ajax_id_product_<?=$arElement["ID"]?>" title="В корзину" />
<div>
    <noindex>
	<a class="button_compare" href="<?echo $arElement["COMPARE_URL"]?>" rel="nofollow"><?echo GetMessage("CATALOG_COMPARE")?></a>&nbsp;
    </noindex>
</div>
</div>
...
<?
$expl = explode('</p>',$arResult["DESCRIPTION"]);
$x=0;
$count=count($expl);
...
<div class="cat_desc bordercolor bgcolor" id="category_description_full" style="background-color:#fff;text-align:justify">
		...
		<div id="category_description_full1"  style="display: none;">
		
<?while ($x++<$count)echo $expl[$x];?></div>
<a href="#" onclick=" $('#category_description_full1').show(); $(this).hide(); return false;" class="lnk_more">еще...</a>
</div>
