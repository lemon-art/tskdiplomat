...
function updateBasketTable(basketItemId, res)
{
    ...
    //минимальная сумма
        ...
        if(parseFloat(res['BASKET_DATA']['allSum']) < 2000){
           BX.addClass('checkout','disabled');
           BX.removeClass('checkout_span','disp');
           BX.adjust(BX('checkout'), {props: {disabled: true}});
        }else{
           console.log('>2000'); 
           BX.removeClass('checkout','disabled');
           BX.addClass('checkout_span','disp');
           BX.adjust(BX('checkout'), {props: {disabled: false}});
        }     
