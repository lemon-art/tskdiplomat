...
<form action="<?=$APPLICATION->GetCurPage()?>" method="get">
	<table class="data-table" cellspacing="0" cellpadding="0" border="0">
		<thead>
		<tr>
			<td style="text-align:center;" valign="top"><input type="submit" class="button" value="<?=GetMessage("CATALOG_REMOVE_PRODUCTS")?>" /></td>
			<?foreach($arResult["ITEMS"] as $arElement):?>
				<td style="text-align:center;" valign="top" width="<?=round(100/count($arResult["ITEMS"]))?>%">
		...
		<tr>
			...
			<?foreach($arResult["ITEMS"] as $arElement):?>
				<td valign="top" style="vertical-align: middle; text-align: center;">
					<?switch($code):
						case "NAME":
		...
		<?foreach($arResult["SHOW_PROPERTIES"] as $code=>$arProperty):
			...
			if($diff || !$arResult["DIFFERENT"]):?>
				<tr>
					...
					<?foreach($arResult["ITEMS"] as $arElement):?>
						<?if($diff):?>
                                                <? if(!$arElement["DISPLAY_PROPERTIES"][$code]["DISPLAY_VALUE"]) continue; ?>
						<? if($caption === false ){ ?>
