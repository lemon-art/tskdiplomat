...
?>
<div class="bx_horizontal_menu_advaced" id="<?=$menuBlockId?>" style="/*max-width:900px*/">
	<ul id="ul_<?=$menuBlockId?>">
	<?foreach($arResult["MENU_STRUCTURE"] as $itemID => $arColumns):?>     <!-- first level-->
		<?$existPictureDescColomn = ($arResult["ALL_ITEMS"][$itemID]["PARAMS"]["picture_src"] || $arResult["ALL_ITEMS"][$itemID]["PARAMS"]["description"]) ? true : false;?>
		<li class="bx_hma_one_lvl <?if($arResult["ALL_ITEMS"][$itemID]["SELECTED"]):?>current<?endif?><?if (is_array($arColumns) && count($arColumns) > 0):?> dropdown<?endif?>" <?if (is_array($arColumns) && count($arColumns) > 0):?>onmouseover="menuCatalogAlignPopup(this, '<?=$menuBlockId?>');"<?endif?>>
				...
				<div class="bx_children_block advanced">
					<div class="bx_children_advanced_panel">
						<span class="bx_children_advanced_panel animate">
							...
							<img src="<?=SITE_TEMPLATE_PATH?>/components/bitrix/menu/catalog_horizontal/images/spacer.png" alt="" style="border: none;">
