...
else
{
	...
	else
	{
		...
		if(count($arResult["PERSON_TYPE"]) == 1)
		{
			...
			{
				?><input type="text" id="PERSON_TYPE_<?= $v["ID"] ?>" name="PERSON_TYPE" value="<?= $v["ID"] ?>"  style="display: none;"><?
			}
			?><input type="text" name="PERSON_TYPE_OLD" value="<?=$arResult["USER_VALS"]["PERSON_TYPE_ID"]?>" style="display: none;"><?
		}
		elseif(count($arResult["PERSON_TYPE"]) > 1)
		{
			?>
		...
		<?if($_POST["is_ajax_post"] != "Y")
		{
			?>
				...
				<br /><br />
				<div align="right">
				<input type="button" name="submitbutton" onclick="submitForm('Y');" value="<?=GetMessage("SOA_TEMPL_BUTTON")?>" class="bt3" style="display:inline-block;width:250px;margin-left: 482px;">
				</div>
			</form>
