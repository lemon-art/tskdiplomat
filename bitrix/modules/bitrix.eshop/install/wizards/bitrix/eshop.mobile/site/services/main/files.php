...
WizardServices::ReplaceMacrosRecursive($_SERVER["DOCUMENT_ROOT"].$siteFolder, Array("SITE_NAME" => htmlspecialcharsbx($wizard->GetVar("siteName"))));
...
while ($arIBlock = $rsIBlock->Fetch())
{
	...
	if(!empty($arIBlock["CODE"])){
		$path_from = $_SERVER['DOCUMENT_ROOT'].$wizard->GetPath().'/site/public/'.LANGUAGE_ID.'/catalog/';
		$path_to = $_SERVER['DOCUMENT_ROOT'].$siteFolder.'catalog/';
	...
	$arNewUrlRewrite[] =
	array(
		"CONDITION"	=>	"#^".$siteFolder."catalog/#",
		"RULE"	=>	"",
		"ID"	=>	"bitrix:catalog",
		"PATH"	=>	 $siteFolder."catalog/index.php",
		);
}
...
COption::SetOptionString("eshop", "siteMobileFolder", $wizard->GetVar("siteFolder"), false, $siteID);
COption::SetOptionString("eshop.mobile", "wizard_installed", "Y", false, WIZARD_SITE_ID);
