    ...
    <? if (!$hide): ?>
        ...
        <? foreach ($arResult["FIELDS"] as $arField): ?>
                    ...
                    <? endforeach ?>
                        ...
                        foreach ($arField["ENUM"] as $v):
                            ...
                            elseif ($arField["USER_TYPE"] == "HTML"):
                                ...
                                <? else: ?>
                                        <textarea cols="" rows="" id="<?= $arField["CODE"] ?>1" name="FIELDS[<?= $arField["CODE"] ?>]" style="height:<?= $arField["USER_TYPE_SETTINGS"]["height"] ?>px;" onblur="if (this.value == '') {
                                                                        this.value = '<?= $arField["DEFAULT_VALUE"]["TEXT"] ?>'}" onclick="if (this.value == '<?= $arField["DEFAULT_VALUE"]["TEXT"] ?>') {
                                                                                    this.value = ''
                                                                                }"><?= $arField["DEFAULT_VALUE"]["TEXT"] ?></textarea>
                                    <? endif; ?>
                                </div>
                                    <? /* DATE */ ?>
            ...
            <? elseif ($arField["TYPE"] == "E"): ?>
                                    ...
                                    <? if ($arField["PROPERTY"]["MULTIPLE"] == "Y"): ?>
                                        <? foreach ($arField["LINKED_ELEMENTS"] as $arEl): ?>
                                            <p class="alx_feed_back_form_checkbox">
                                                <input type="checkbox" name="FIELDS[<?= $arField["CODE"] ?>][]" value="<?= $arEl["ID"] ?>" id="<?= $arField["CODE"] ?>1_<?= $arEl["ID"] ?>" <? if (!empty($_POST["FIELDS"][$arField["CODE"]]) && in_array($arEl["ID"], $_POST["FIELDS"][$arField["CODE"]])): ?>checked="checked"<? endif; ?>/>
                ...
                <? else: ?> 
                                        <input type="text" size="40" id="<?= $arField["CODE"] ?>1" name="FIELDS[<?= $arField["CODE"] ?>]" value="<?= $arField["DEFAULT_VALUE"] ?>" class="alx_feed_back_form_inputtext <?php if ($arField["CODE"] == 'PHONE_FID1') echo "mask" ?>" />
                                    <? endif; ?>
                                </div>
                                    <? /* FILE */ ?>
                                <? elseif ($arField["TYPE"] == "F"): ?>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $("a").click(function () {
                ...
                $(".alx_feedback_popup").fancybox({ajax: {type: "POST", data: "OPEN_POPUP_" + a + "=Y"}, titleShow: !1, type: "ajax", href: "", afterShow: function () {
                        "undefined" != typeof AltasibFeedbackOnload_<?= $ALX ?> && AltasibFeedbackOnload_<?= $ALX ?>()
