...
if ($this->StartResultCache(false, array($arrFilter, ($arParams["CACHE_GROUPS"] === "N" ? false : $USER->GetGroups())))) {
    ...
    if (isset($arParams["LINKED_ELEMENTS_ARR"][0])):
        $count = $arParams["LINKED_ELEMENTS_ARR"];
        $i=0;
        foreach ($arParams["LINKED_ELEMENTS_ARR"] as $iblock){
            if($i > ($arParams["ELEMENT_COUNT"]-1)) break;
            $temp=rand(0,$count);
            if(!array_search($iblock["ID"], $mas))
            {
                $mas[] = $iblock["ID"];
                $i++;
            }
    ...
    if ($arParams["CODE"]) {
        $arParams["ID"] = CIBlockFindTools::GetElementID(
                        $arParams["ID"], $arParams["CODE"], false, false, array(
